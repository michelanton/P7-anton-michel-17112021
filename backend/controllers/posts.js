...
exports.allPosts= (req, res, next) => {  
    let sql = "SELECT posts.title, users.pseudo, posts.description, users.id, posts.id AS post_id, users.avatar_url, posts.user_id, posts.date, posts.media_url  FROM posts INNER JOIN users ON posts.user_id = users.id ORDER BY posts.date DESC ";
...
// GET d'un post unique : postCommentaire.vue + superUserComments.vue / axios thePost()
exports.postIdCtrl = (req, res, next) => {
    console.log("reParamssID", req.params.id)
    let sqlPostId = "SELECT posts.id AS postID, posts.title, posts.description, posts.media_url, users.id, users.avatar_url, users.pseudo, posts.date AS postDate FROM posts INNER JOIN users ON posts.user_id = users.id WHERE posts.id = ?"
...
exports.writeCommentCtrl = (req, res, next) => {
  ...
  let sqlWriteComment = "INSERT INTO comments VALUES (NULL, ?, ?, ?, NOW())";
